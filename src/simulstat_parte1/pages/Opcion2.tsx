...
export default function Opcion2() {
  ...
  const run = () => {
    const N = Number(n);
    const I = Number(intervalos);
    if (!Number.isFinite(N) || !Number.isFinite(I) || N <= 0 || I <= 0) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      let x = 0;
      if (R1 < A1) x = (15 + Math.sqrt(8 * R2 + 1)) / 2;
      else x = (21 - Math.sqrt(-8 * R2 + 9)) / 2;

      outX.push(x);
      outRows.push({ R1, R2, x });
    }
    ...
    let pdf = xs.map((v) => {
      if (v <= 9) return v / 2 - 15 / 4;
      if (v < 10) return -v / 2 + 21 / 4;
      return 0;
    });
    ...
    setRows(outRows);
    setResult(`Resultado: ${(A1 + A2).toFixed(6)}`);
  };
  ...
  return (
    <div className="bg-white border rounded-lg p-6 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold">Opción 2 (Ejemplo 2)</h2>
          <p className="text-sm text-gray-500">Histograma vs PDF teórica</p>
        </div>
      ...
      {showTable && (
        <DataTableModal
          title="Tabla de resultados (Opción 2)"
          rows={rows}
          onClose={() => setShowTable(false)}
          fileName="opcion2_resultados.csv"
        />
      )}
    </div>
  );
